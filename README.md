...
The API builds with the oxc toolchain (tsdown, powered by rolldown + the oxc transformer), orchestrated by Nx. tsdown reads `apps/backend/tsconfig.app.json`, so `experimentalDecorators` and `emitDecoratorMetadata` are honored - NestJS dependency injection and route reflection keep working with `reflect-metadata`.
...
This runs tsdown (the oxc toolchain) via Nx and emits `dist/apps/backend/main.cjs`. Run it directly with `node dist/apps/backend/main.cjs`.
...
nestjs-template/
|- apps/
|  |- backend/             # NestJS application (oxc/tsdown build orchestrated by Nx)
...
| ------- | ----------------------- | ------------- |
| backend | scope:backend, type:app | scope:shared  |
...
```sh
npx nx run backend:build   # second run: < 100 ms from cache
```
...
npx nx g @nx/nest:resource --project=backend --name=users
```

### Module Boundaries

Enforce architectural rules at lint time. A `scope:backend` library can never accidentally import from another app - only from `scope:shared` libs. Configure the rules in `eslint.config.mjs`.

### Docker

The `apps/backend/Dockerfile` uses a multi-stage build:

1. **builder** - installs all deps, builds with `nx run backend:prune` (the oxc/tsdown build plus a pruned lockfile and workspace modules)
2. **runner** - minimal Node 22 Alpine image with only production deps

```sh
docker build -f apps/backend/Dockerfile -t nestjs-backend .
